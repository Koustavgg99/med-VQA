...
quesntion_types = ['Organ', 'Color', 'Shape', 'Quantity', 'Position', 'Modality', 'Abnormality', 'Plane', 'KG', 'Size']
...
def parse_args():
    ...
    parser.add_argument('--SLAKE_dir', type=str, default=r'D:\PhD\Project\Slake\Slake1.0',
                        help='SLAKE dir')
...
if __name__ == '__main__':
    ...
    if args.use_SLAKE:
        ...
        eval_dset = dataset_SLAKE.VQAFeatureDataset(args.split, args, dictionary)
    ...
    def save_activation_map(args, model, dataloader):
        ...
        for image_name, v, qid, q, a, ans_type, q_types, p_type in iter(dataloader):
            ...
            act_image = os.path.join(args.SLAKE_dir, "imgs", image_name[0])
            ...
            print("image name: {}".format(image_name[0].split('/')[0]))
            new_image = image_name[0].split('/')[0]+'_'+str(qid.item())+'_map.%s'%'jpg'
            if batch_score == 1.0:
