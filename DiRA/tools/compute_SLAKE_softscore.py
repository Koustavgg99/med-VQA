...
def filter_answers(answers_dset, min_occurence):
    ...
    for ans_entry in answers_dset:
        if ans_entry['q_lang'] != 'en':
            chi_count+=1
            #print("Question answer in chinese encountered")
            continue
    ...
    print("There are {} chinese questions in Train".format(chi_count))
    return occurence
...
if __name__ == '__main__':
    ...
    train_answers = json.load(open(os.path.join(Slake_dir, 'train.json'), encoding="utf8")) # list of dictionary
    ...
    cache_path = os.path.join(Slake_dir, 'cache', 'trainval_ans2label.pkl')
    if os.path.isfile(cache_path):
        print('found %s' % cache_path)
        ans2label = cPickle.load(open(cache_path, 'rb'))
    else:
        ans2label = create_ans2label(occurence, 'trainval', os.path.join(Slake_dir, 'cache'))

    compute_target(train_answers, ans2label, 'train', os.path.join(Slake_dir, 'cache'))

    test_answers = json.load(open(os.path.join(Slake_dir, 'test.json'), encoding="utf8"))
    compute_target(test_answers, ans2label, 'test', os.path.join(Slake_dir, 'cache'))

    validate_answers = json.load(open(os.path.join(Slake_dir, 'validate.json'), encoding="utf8"))
    compute_target(validate_answers, ans2label, 'validate', os.path.join(Slake_dir, 'cache'))
