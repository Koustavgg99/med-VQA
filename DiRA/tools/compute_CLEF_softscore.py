...
def filter_answers(answers_dset, min_occurence):
    ...
    """
    occurence = {}
    for index, row in answers_dset.iterrows():
        answers = row['answers']
        gtruth = row['answers']
    ...
    occurence['others'] = set()
    occurence['others'].add(-1)
...
def compute_target(answers_dset, ans2label, name, cache_root='data_RAD/cache'):
    ...
    ***answers_dset should be preprocessed***
    ...
    count = 0
    for index, row in answers_dset.iterrows():
        answers = preprocess_answer(row['answers'])
        # answer_count = {}
        ...
        if answers in ans2label:
            ...
            print("{} not in ans2label".format(answers))
            scores.append(1.)
            labels.append(ans2label['others'])
            #count+=1
            #print("{} not in ans2label".format(answers))
            #continue
...
if __name__ == '__main__':
    ...
    train_answers = pd.read_csv(os.path.join(CLEF_dir, 'All_Train.csv'), sep='|', names=['image_name','qid','questions','answers','question_type','answer_type']) # list of dictionary
    ...
    cache_path = os.path.join(CLEF_dir, 'new_cache', 'trainval_ans2label.pkl')
    if os.path.isfile(cache_path):
        print('found %s' % cache_path)
        ans2label = cPickle.load(open(cache_path, 'rb'))
    else:
        ans2label = create_ans2label(occurence, 'trainval', os.path.join(CLEF_dir, 'new_cache'))
    
    target = compute_target(train_answers, ans2label, 'train', os.path.join(CLEF_dir, 'new_cache'))
    print(len(target))

    CLEF_test_dir = 'D:\PhD\Project\VQA_Dataset'
    test_answers = pd.read_csv(os.path.join(CLEF_test_dir, 'All_Test.csv'), sep='|', names=['image_name','answer_type','qid', 'question_type', 'question', 'answers'])
    test_target = compute_target(test_answers, ans2label, 'test', os.path.join(CLEF_test_dir, 'new_cache'))
    print(len(test_target))
    
    CLEF_valid_dir = 'D:\PhD\Project\VQA_Dataset'
    validate_answers = pd.read_csv(os.path.join(CLEF_valid_dir, 'All_Val.csv'), sep='|', names=['image_name','qid','questions','answers','question_type','answer_type'])
    valid_target=compute_target(validate_answers, ans2label, 'validate', os.path.join(CLEF_valid_dir, 'new_cache'))
    print(len(valid_target))
    
    CLEF_dir = r'D:\PhD\Project\VQA_Dataset\new_cache\trainval_ans2label.pkl'
    ans2label = cPickle.load(open(os.path.join(CLEF_dir), 'rb'))
    print(ans2label.keys())
