...
def parse_args():
    ...
    parser.add_argument('--RAD_dir', type=str, default=r'D:\PhD\Project\VQA_RAD',
                        help='RAD dir')
...
def get_result(model, dataloader, device, args):
    ...
    with torch.no_grad():
        for _, v, qid, q, a, ans_type, q_types, p_type in iter(dataloader):
            ...
            print("Qid: {}, score: {}".format(qid, batch_score))
...
if __name__ == '__main__':
    ...
    def save_questiontype_results(outfile_path, quesntion_types_result, epoch):
        for i in quesntion_types_result:
            pd.DataFrame(quesntion_types_result[i]).transpose().to_csv(os.path.join(outfile_path , 'question_type_{}_{}.csv'.format(i,epoch)))
    # Testing process
    ...
    def save_activation_map(args, model, dataloader):
        ...
        for image_name, v, qid, q, a, ans_type, q_types, p_type in iter(dataloader):
            ...
            act_image = os.path.join(args.RAD_dir, "images",image_name[0])
            ...
            new_image = image_name[0].split('.')[0]+'_'+str(qid.item())+'_map.%s'%image_name[0].split('.')[1]
            if batch_score == 1.0:
